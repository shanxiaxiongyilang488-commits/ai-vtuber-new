...
function parseReasoningEffort(value: unknown): LabChatRequest['reasoningEffort'] {
  return value === 'high' ? 'high' : value === 'low' ? 'low' : undefined;
}
...
async function parseRequest(request: Request): Promise<{ body: LabChatRequest; images: ImageInput[]; enableMemoryByDefault: boolean }> {
  ...
  if (ct.includes('multipart/form-data')) {
    ...
    const body: LabChatRequest = {
      ...
      reasoningEffort: parseReasoningEffort(fd.get('reasoningEffort')),
  ...
  body.reasoningEffort = parseReasoningEffort(body.reasoningEffort);
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    const configured = await runCharacterAi({
      ...
      images,
      maxTokens,
	  reasoningEffort: route === 'chat' ? body.reasoningEffort : undefined,
