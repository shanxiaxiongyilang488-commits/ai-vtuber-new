...
const MEMORY_REVIEW_SCHEMA = {
  ...
  required: ['summary', 'reflectionComment', 'importance', 'candidates', 'actionCandidate'],
};
...
const SYSTEM_PROMPT = [
  ...
  'Separately produce actionCandidate by semantically reviewing the completed user/assistant exchange. It is advisory structured data, never an execution command.',
  'Do not infer an action from keywords. Distinguish requests for current external information from historical discussion, and explicit generation requests from opinions or hypotheticals.',
  'Use WEB_SEARCH only when answering the user requires fresh or externally retrieved web information. Use MEMORY_LOOKUP when the user asks to recall prior character conversations, creations, or stored experiences.',
  'Use IMAGE, VIDEO, VIDEO_EDIT, VOICE, MANGA, or YAML only for a clear request to perform that action. Otherwise return needAction=false and suggestedAction=NONE.',
  'query is a concise proposed lookup/search query and is required for WEB_SEARCH or MEMORY_LOOKUP. Never include hidden reasoning or chain of thought.',
  'Return exactly one JSON object. No markdown and no extra text.',
	'Schema: {"summary":"","reflectionComment":"","importance":0.0,"candidates":[{"title":"","summary":"","reason":"","importance":0.0,"tags":[],"evidenceRefs":[]}],"actionCandidate":{"needAction":false,"suggestedAction":"NONE","confidence":0.0,"reason":"","query":""}}',
].join('\n');
...
function normalizeReview(
  ...
  return {
	...
	actionCandidate: normalizeActionCandidate(data.actionCandidate),
	technicalEvidence,
    cost: {
      provider: 'gemini',
      model,
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    console.log('[ThoughtAction]', result.actionCandidate);
    return json(result);
  } catch (error) {
