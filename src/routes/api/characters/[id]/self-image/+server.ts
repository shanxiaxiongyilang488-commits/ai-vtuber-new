...
import { generateFalImage } from '$lib/server/mediaProviders/fal';
...
type SelfImageProvider = 'nanobanana-2-lite' | 'nano-banana-2' | 'nano-banana-pro' | 'gpt-image-2' | 'ideogram-character';
...
function isSelfImageProvider(value: unknown): value is SelfImageProvider {
  return value === 'nanobanana-2-lite'
    || value === 'nano-banana-2'
...
function imageModelForProvider(provider: SelfImageProvider): string {
  if (provider === 'nanobanana-2-lite') return 'nanobanana-2-lite';
  if (provider === 'nano-banana-2') return 'nano-banana-2';
  if (provider === 'nano-banana-pro') return 'nano-banana-pro';
  if (provider === 'ideogram-character') return 'ideogram-v3';
  return 'gpt-image-2';
}
...
async function generateImage(provider: SelfImageProvider, input: ImageGenerationInput): Promise<string> {
  const images = provider === 'nanobanana-2-lite'
    ? await generateFalImage(input)
    : provider === 'gpt-image-2'
    ? await generateOpenAIImage(input)
    : provider === 'ideogram-character'
      ? await generateIdeogramImage(input)
      : await generateGeminiImage(input);
...
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    ...
    const provider: SelfImageProvider = isSelfImageProvider(body?.provider) ? body.provider : 'nano-banana-2';
