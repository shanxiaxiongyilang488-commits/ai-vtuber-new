<script lang="ts">
  ...
  let todayVideoUsage = $state({ generationCount: 0, estimatedCostUsd: 0 });
  ...
  onMount(() => {
    ...
    void loadTodayVideoUsage();
  });

  async function loadTodayVideoUsage(): Promise<void> {
    try {
      const response = await fetch('/api/video/usage', { cache: 'no-store' });
      const data = await response.json();
      if (response.ok && data?.usage) todayVideoUsage = data.usage;
    } catch {
...
<div class="anime-lab">
  ...
  <aside class="daily-video-usage" aria-label="本日のFAL動画利用状況">
    <strong>本日のFAL動画利用</strong>
    <span>動画生成回数: {todayVideoUsage.generationCount}</span>
    <span>推定消費額: ${todayVideoUsage.estimatedCostUsd.toFixed(2)}</span>
  </aside>
...
<style>
  .daily-video-usage { max-width: 1400px; margin: 0 auto 14px; display: flex; gap: 18px; align-items: center; padding: 10px 14px; border: 1px solid rgba(34,211,238,.25); border-radius: 8px; background: rgba(8,15,32,.86); color: #cbd5e1; font-size: 12px; }
  .daily-video-usage strong { color: #67e8f9; margin-right: auto; }
  :global(body) {
    margin: 0;
    background: #05070d;
    color: #e8eef8;
