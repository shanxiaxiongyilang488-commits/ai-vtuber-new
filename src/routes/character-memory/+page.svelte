<script lang="ts">
  ...
  type MonologueThought = { emoji: string; text: string };
  ...
  let aiProfile = $state({ brainAI: 'AUTO', conversationAI: 'INHERIT', storyCardAI: 'INHERIT', motionPromptAI: 'INHERIT', characterAnalysisAI: 'INHERIT', intentRouterAI: 'INHERIT', imageAI: 'GPT Image', videoAI: 'Seedance2', voiceAI: 'Irodori', memoryEnabled: true });
  ...
  async function appendMessage(
    ...
    structured: { storyCard?: StoryCardDesign; videoPackage?: VideoPackage; motionPrompt?: string; aiModels?: AIModelMetadata } = {},
  ): Promise<void> {
    if (projectOnlyMode) {
      const now = new Date().toISOString();
      messages = [...messages, {
  ...
  $effect(() => {
    ...
    fetch(`/api/voice/speak?characterId=${encodeURIComponent(id)}`)
      ...
      .then((data) => {
        if (character?.id === id) voiceConfig = (data?.voice as CharacterVoiceConfig | null) ?? null;
      })
      .catch(() => {
        voiceConfig = null;
      });
  ...
  async function speakMessage(message: ChatMessage): Promise<void> {
    ...
    const state = voiceStateByMessageId[message.id];
    if (state === 'loading' || state === 'playing') return;
    try {
      let audioUrl = voiceUrlByMessageId[message.id];
      if (!audioUrl) {
        voiceStateByMessageId = { ...voiceStateByMessageId, [message.id]: 'loading' };
        const response = await fetch('/api/voice/speak', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ characterId: character.id, text: stripInlineImageData(message.text) }),
        });
        const data = await response.json();
        ...
        // 生成済み音声の URL だけをメモリに保持する (チャットログへは保存しない)。
        voiceUrlByMessageId = { ...voiceUrlByMessageId, [message.id]: audioUrl };
      }
      currentVoiceAudio?.pause();
      const audio = new Audio(audioUrl);
      currentVoiceAudio = audio;
      voiceStateByMessageId = { ...voiceStateByMessageId, [message.id]: 'playing' };
      ...
      const { [message.id]: _finished, ...rest } = voiceStateByMessageId;
      voiceStateByMessageId = rest;
    } catch (error) {
      console.warn('[VOICE_SPEAK_ERROR]', error instanceof Error ? error.message : String(error));
      voiceStateByMessageId = { ...voiceStateByMessageId, [message.id]: 'error' };
    }
  }
  ...
  function autoSpeakLatestReply(): void {
    ...
    if (latest) void speakMessage(latest);
  }
  ...
  /** Gemini Omni Flash Direct Mode の発動条件。
  ...
  function hasDirectVideoTrigger(text: string, _attachmentCount: number): boolean {
    return selectedVideoModelIdFromProfile() === 'gemini-omni-flash-reference'
      && isExplicitVideoGenerationRequest(text);
  }
  ...
  async function generateDirectVideoFromReferences(text: string, queuedImages: AttachImage[]): Promise<boolean> {
    ...
    if (!videoModel || videoModel.id !== 'gemini-omni-flash-reference') {
      throw new Error('Gemini Omni Flash Direct Mode is only available for Gemini Omni Flash Reference.');
    }
    const prompt = text;
    const projectVideoReferences = projectOnlyMode
      ...
      : [];
	const animationSheetReferences = [...projectVideoReferences, ...queuedImages.map((image) => image.dataUrl)];
	if (!(await confirmVideoProductionMaterials(animationSheetReferences))) return false;
    const imageSources = [...new Set([
      ...projectVideoReferences,
    ...
    if (!prompt.trim()) throw new Error('Gemini Omni Flash Direct Mode requires a user prompt.');
    if (referenceImageCount === 0) throw new Error('Gemini Omni Flash Direct Mode requires at least one attached image.');

    const requestPayload = {
      modelId: 'gemini-omni-flash-reference',
    ...
    console.log('[GEMINI_OMNI_FLASH_DIRECT_MODE]', {
      VIDEO_MODE: 'GEMINI_OMNI_FLASH_DIRECT_MODE',
    ...
    console.log('[GEMINI_OMNI_FLASH_OUTGOING_PAYLOAD]', {
    ...
    try {
      ...
      const result = JSON.parse(responseBody) as { url?: string; requestId?: string; queueResponse?: unknown; source?: string; model?: string };
      if (!result.url) throw new Error('FAL response did not include a video URL.');
      const isMockVideoPreview = result.url.startsWith('/mock/') || result.url.includes('/mock/');
      console.log('[GEMINI_OMNI_FLASH_DIRECT_MODE_RESPONSE]', {
        falModelId: result.model ?? videoModel.falModel,
        requestId: result.requestId ?? null,
        queueResponse: result.queueResponse ?? null,
        videoUrl: result.url,
        resultType: isMockVideoPreview ? 'MOCK_VIDEO_PREVIEW' : 'FAL_VIDEO',
      });
      await appendMessage('user', prompt);
      await appendMessage('assistant', `Gemini Omni Flash Direct Mode\n\nPrompt:\n${prompt}`);
      ...
      const detail = error instanceof Error ? error.message : String(error);
      const failure = `FAL API Error\nrequestPayload: ${JSON.stringify(requestPayload, null, 2)}\nselectedVideoModel: ${videoModel.id}\nreferenceImageCount: ${referenceImageCount}\n${detail}`;
      console.error('[GEMINI_OMNI_FLASH_DIRECT_MODE_ERROR]', { FAL_API_Error: detail, requestPayload, selectedVideoModel: videoModel.id, referenceImageCount });
  ...
  async function generateDirectSeedanceAnimation(messageId: string, inputPackage: VideoPackage, preferredImages: string[] = []): Promise<void> {
    ...
    try {
	  ...
	  const duration = videoModel.falModel.includes('gemini-omni-flash')
	    ...
	    : isSeedanceMini ? safeDuration : Math.max(1, Math.min(15, requestedDuration));
      ...
      const generated = JSON.parse(body) as { url?: string };
  ...
  function resetThinkingStream(): void {
    stopMonologueTimer();
    monologueQueue = [];
    thinkingStream = [];
  ...
  function fallbackMonologues(context: { goalText: string; relationshipHits: RelationshipSearchHit[]; emotion: BrainEmotionState | null }): MonologueThought[] {
    ...
    if (lastImageValidationMissing.length > 0) {
      thoughts.push({ emoji: '😳', text: `また${featureMonologueNames(lastImageValidationMissing)}を間違えたら恥ずかしいな…` });
    }
    const relation = context.relationshipHits[0]?.value?.replace(/[。.]+$/u, '');
    if (relation) thoughts.push({ emoji: '😊', text: `${THINKING_USER_NAME}は${relation}…だったよね…` });
    const goal = context.goalText.trim().slice(0, 30);
    if (goal) thoughts.push({ emoji: '🤔', text: `${goal}…どんなふうにしようかな…` });
    if (thoughts.length === 0) thoughts.push({ emoji: '💭', text: 'なにから考えよう…' });
    return thoughts;
  }
  ...
  async function fetchInnerMonologue(context: {
    ...
    try {
      ...
      console.log('[INNER_MONOLOGUE_JSON]', { count: thoughts.length, model: data?.model });
      enqueueMonologues(thoughts);
    } catch (error) {
  ...
  async function runImageGenerationReasoning(queryText: string): Promise<void> {
    ...
    if (thinkingStreamDone) resetThinkingStream();
    ...
    try {
      intentDecision = confirmedIntentDecision ?? await classifyCharacterMemoryIntent(text, queuedImages, queuedYaml);
    } catch (error) {
    ...
    // ⚡ Fast Mode V1: 通常会話は CHAT_FAST（Story系をスキップ）、STORYキーワード時のみ STORY。
	    mode = 'CHAT_FAST';
    ...
    // ⑤ CHAT_FAST でも AI返答 / Thinking / Emotion / Growth / Memory参照 は維持（上記 apply* と systemPrompt で実行済み）。
    // ⑦ デバッグログ。
    ...
    if (routedIntent === 'IMAGE') {
      ...
      try {
        ...
        try {
          ...
          await appendMessage('assistant', '1枚絵を生成しました。', savedUrl);
        } finally {
          generatingPortrait = false;
        }
      } catch (error) {
    ...
    try {
      ...
      if (analyzedStoryCard) {
        ...
        if (imageAction) {
          ...
          await appendMessage(
            'assistant',
            storyMode || planningMode ? reply : applyEmotionInfluence(reply, currentEmotion),
			[],
			{ aiModels: responseAIModels },
          );
      ...
      await captureSelfTalkMemory(text, assistantMemoryText);
      clearAttachQueue();
  ...
  async function captureSelfTalkMemory(userText: string, assistantText: string): Promise<void> {
  ...
  async function generateSelfPortrait(text: string): Promise<void> {
    ...
    try {
      ...
      await appendMessage('assistant', '自分を描いてみたよ。これが今のわたし。', savedUrl);
    } catch (error) {
      errorMessage = error instanceof Error ? error.message : String(error);
    } finally {
      generatingPortrait = false;
            ...
            <article class:user={message.role === 'user'} class:assistant={message.role === 'assistant'}>
              <div class="msg-id">
                ...
                {#if message.role === 'assistant'}
				  ...
				  <span class="msg-ai">
					<span class="msg-ai-icon">AI</span>{displayedAIModels.conversation ?? displayedAIModels.storyCard ?? displayedAIModels.imageAnalysis ?? AI_ROLE_MODELS.conversation}
                  </span>
                {:else}
