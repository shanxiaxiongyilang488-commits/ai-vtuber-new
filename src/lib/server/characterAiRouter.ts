import { chatOpenAIWithModel, OPENAI_ORCHESTRATION_MODEL } from '$lib/providers/openai';
...
import { openAIModelForSelection, resolveCharacterAiProvider, type CharacterAiRole } from '$lib/server/characterSettings';
...
export async function runCharacterAi(input: {
	...
	const model = input.provider === 'openai' && input.model
		? input.model
		: openAIModelForSelection(selection) ?? OPENAI_ORCHESTRATION_MODEL;
	const result = await chatOpenAIWithModel({ ...common, apiKey: settings.openai.key, model });
	return { text: result.text, provider: 'openai', model: result.model, selection: input.provider === 'openai' ? model : (selection === 'AUTO' ? 'GPT-5.5' : selection) };
}
