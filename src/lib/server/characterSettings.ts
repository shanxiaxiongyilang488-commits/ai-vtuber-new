...
import { isReflectionTendency, type ReflectionTendency } from '$lib/ai/reflection-router/types';
...
export interface CharacterAiProfile {
  ...
  reflectionTendency: ReflectionTendency;
}
...
export const AI_PROVIDERS = ['AUTO', 'GPT-5.5', 'GPT-5.6 Terra', 'GPT-5.6 Luna', 'GPT-5.6 Sol', 'Grok', 'Gemini', 'Claude', 'Local LLM'] as const;
...
export const IMAGE_AI_PROVIDERS = ['GPT Image', 'Anima', 'NanoBanana 2 Lite', 'NanoBanana 2', 'NanoBanana Pro', 'NanoBanana', 'Flux Kontext', 'Seedream'] as const;
export type ImageAiProvider = (typeof IMAGE_AI_PROVIDERS)[number];
export const VIDEO_AI_PROVIDERS = ['MiniMax H3', 'Seedance2 Mini', 'Seedance2', 'Sora 2', 'Kling 1.6', 'Kling Elements', 'Kling O1 Reference', 'Kling 1.6 Elements', 'Gemini Omni Flash Reference', 'Gemini Omni Flash Image', 'Gemini Omni Flash Edit'] as const;
export type VideoAiProvider = (typeof VIDEO_AI_PROVIDERS)[number];
export const VOICE_AI_PROVIDERS = ['Honoka', 'Irodori'] as const;
export type VoiceAiProvider = (typeof VOICE_AI_PROVIDERS)[number];

const DEFAULT_PROVIDER: AiProvider = 'GPT-5.6 Terra';
const DEFAULT_PROFILE: CharacterAiProfile = {
  brainAI: 'GPT-5.6 Terra', conversationAI: 'INHERIT', storyCardAI: 'INHERIT', motionPromptAI: 'INHERIT',
  characterAnalysisAI: 'INHERIT', intentRouterAI: 'INHERIT', imageAI: 'GPT Image', videoAI: 'MiniMax H3', voiceAI: 'Irodori', memoryEnabled: true, reflectionTendency: 'balanced',
};
...
function normalizeProfile(value: unknown, legacyProvider: unknown): CharacterAiProfile {
  ...
  const effectiveBrainAI: AiProvider = brainAI === 'AUTO' ? 'GPT-5.6 Terra' : brainAI;
  const hasExplicitBrain = isValidProvider(profile.brainAI);
  return {
    brainAI: effectiveBrainAI,
    ...
    reflectionTendency: isReflectionTendency(profile.reflectionTendency) ? profile.reflectionTendency : DEFAULT_PROFILE.reflectionTendency,
  };
}
...
export function openAIModelForSelection(selection: string): string | undefined {
  if (selection === 'GPT-5.5') return 'gpt-5.5';
  if (selection === 'GPT-5.6 Terra') return 'gpt-5.6-terra';
  if (selection === 'GPT-5.6 Luna') return 'gpt-5.6-luna';
  if (selection === 'GPT-5.6 Sol') return 'gpt-5.6-sol';
  return undefined;
}
