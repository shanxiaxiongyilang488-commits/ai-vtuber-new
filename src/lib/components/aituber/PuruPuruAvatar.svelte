<script lang="ts">
  ...
  import { getPuruPuruModel, loadPuruPuruPackage, nextBlinkDelayMs, PuruPuruRenderer, randomBlinkStep, savePuruPuruModel } from '$lib/purupuru';
  ...
  interface Props {
    ...
    thinking?: boolean;
    modelKey?: string;
    /** フォールバック画像のトリミング。'cover'=枠いっぱい(アイコン用)、'contain'=全身が切れない(ステージ用)。 */
    fit?: 'cover' | 'contain';
    onRendererChange?: (renderer: 'css-fallback' | 'purupuru-runtime') => void;
  }

  let { src, alt, speaking = false, mouthLevel = 0, emotion = 'neutral', thinking = false, modelKey = 'default', fit = 'cover', onRendererChange }: Props = $props();
  ...
  let modelName = $state('');
  let loadError = $state('');
  // CSSフォールバック用の簡易瞬き。タイミング分布は公式移植のrendererSettingsを共用する。
  let blinkClosed = $state(false);
  ...
  onMount(() => {
    ...
    const scheduleBlink = () => {
      blinkTimer = setTimeout(() => {
        blinkClosed = true;
        const step = randomBlinkStep();
        blinkTimer = setTimeout(() => {
          blinkClosed = false;
          scheduleBlink();
        }, step.closeMs + step.holdMs + step.openMs);
      }, nextBlinkDelayMs());
    };
    scheduleBlink();
  ...
  async function fetchBundledModel(): Promise<{ blob: Blob; name: string; warning?: string } | null> {
    const attemptedUrls: string[] = [];
    const requestedName = `${modelKey}.purupuru`;
    const candidates = [...new Set([requestedName, 'default.purupuru'])];
    let requestedModelMissing = false;
    for (const name of candidates) {
      const modelUrl = `/purupuru/${encodeURIComponent(name)}`;
      attemptedUrls.push(modelUrl);
      try {
        const response = await fetch(modelUrl);
        if (!response.ok) {
          if (name === requestedName) requestedModelMissing = true;
          continue;
        }
        const blob = await response.blob();
        const head = new Uint8Array(await blob.slice(0, 2).arrayBuffer());
        if (head[0] === 0x50 && head[1] === 0x4b) {
          const warning = requestedModelMissing && name === 'default.purupuru'
            ? `/purupuru/${encodeURIComponent(requestedName)} が見つからないため、default.purupuru を表示しています。`
            : undefined;
          return { blob, name, warning };
        }
        if (name === requestedName) requestedModelMissing = true;
      } catch {
        if (name === requestedName) requestedModelMissing = true;
    ...
    loadError = `PuruPuruモデルが見つかりません。${attemptedUrls.join(' または ')} に .purupuru ファイルを保存してください。`;
    return null;
  }
  ...
  onMount(() => {
    ...
    void (async () => {
      try {
        ...
        if (!disposed && bundled) {
          await activateModel(bundled.blob, bundled.name);
          if (bundled.warning) loadError = bundled.warning;
        }
      } catch (error) {
...
<div class="purupuru-avatar" class:speaking class:thinking data-renderer={runtimeActive ? 'purupuru-runtime' : 'css-fallback'} data-emotion={emotion} style={`--mouth-level:${level};--talk-speed:${Math.max(105, 230 - level * 100)}ms`} role="img" aria-label={`${alt}: ${speaking ? 'speaking' : thinking ? 'thinking' : emotion}`}>
  ...
  {#if !runtimeActive}
    <div class="float-layer"><div class="deform-layer"><img {src} alt="" draggable="false" class:blinking={blinkClosed} class:fit-contain={fit === 'contain'} /></div></div>
  ...
  {#if loadError}<span class="model-error" title={loadError} role="alert">{loadError}</span>{/if}
</div>
...
<style>
  ...
  canvas { position:absolute; inset:0; display:none; width:100%; height:100%; object-fit:contain; transform-origin:center 82%; }
  canvas.visible { display:block; }
  .thinking canvas.visible { animation:thinking-purupuru 1.8s ease-in-out infinite; }
  ...
  img { display:block; width:100%; height:100%; object-fit:cover; user-select:none; transform-origin:center 35%; transition:transform 70ms ease-out; }
  img.fit-contain { object-fit:contain; }
  img.blinking { transform:scaleY(.94); }
  ...
  .model-error { position:absolute; left:5px; right:5px; bottom:28px; z-index:3; padding:5px 7px; border:1px solid rgba(248,113,113,.55); border-radius:6px; color:#fecaca; background:rgba(69,10,10,.9); font:700 10px/1.35 system-ui,sans-serif; overflow-wrap:anywhere; }
  ...
  @media (prefers-reduced-motion:reduce) { .float-layer,.speaking .float-layer,.thinking .float-layer,.thinking .aura,.thinking canvas.visible{animation:none} img{transition:none} }
</style>
