<script lang="ts">
  ...
  type PersonalityStartPayload = {
    ...
    aiProfile: { brainAI: string; conversationAI: string; storyCardAI: string; motionPromptAI: string; characterAnalysisAI: string; intentRouterAI: string; imageAI: string; videoAI: string; voiceAI: string; memoryEnabled: boolean; reflectionTendency: 'quick' | 'balanced' | 'reflective' };
  };
  ...
  const IMAGE_AI = ['GPT Image', 'Anima', 'NanoBanana 2 Lite', 'NanoBanana 2', 'NanoBanana Pro', 'NanoBanana', 'Flux Kontext', 'Seedream'];
  const VOICE_AI = ['Honoka', 'Irodori'];
  ...
  let {
    ...
    characterName,
    characterImage = '',
    provider = $bindable('GPT-5.6 Terra'),
    ...
    videoAI = $bindable('MiniMax H3'),
    voiceAI = $bindable('Irodori'),
    memoryEnabled = $bindable(true),
    reflectionTendency = $bindable<'quick' | 'balanced' | 'reflective'>('balanced'),
    ...
    reflectionTendency?: 'quick' | 'balanced' | 'reflective';
    mode?: PersonalityMode;
    busy?: boolean;
    onCancel: () => void;
    ...
    try {
      ...
      if (settings.openai?.key) {
        ...
        if (modelsResponse.ok) {
          const payload = await modelsResponse.json() as { models?: Array<{ id?: string }> };
          const available = new Set((payload.models ?? []).map((model) => model.id).filter((id): id is string => Boolean(id)));
          const openAIModelBySelection: Record<string, string> = {
            'GPT-5.5': 'gpt-5.5',
            'GPT-5.6 Terra': 'gpt-5.6-terra',
            'GPT-5.6 Luna': 'gpt-5.6-luna',
            'GPT-5.6 Sol': 'gpt-5.6-sol',
          };
          providerCards = providerCards.map((card) => {
            const model = openAIModelBySelection[card.id];
            return model ? { ...card, implemented: available.has(model) } : card;
          });
        }
      }
    } catch {
  ...
  function startWithConfig(): void | Promise<void> {
    return onStart({
      ...
      aiProfile: { brainAI: provider, conversationAI, storyCardAI, motionPromptAI, characterAnalysisAI, intentRouterAI, imageAI, videoAI, voiceAI, memoryEnabled, reflectionTendency },
    });
  }
...
>
  ...
  >
    ...
    <section class="studio-profile" aria-label="AI Studio Engine profile">
      <label>🧠 Brain AI
        <select value={provider} onchange={(event) => selectProvider(event.currentTarget.value)} disabled={busy}>
          {#each providerCards.filter((card) => card.implemented) as option (option.id)}
