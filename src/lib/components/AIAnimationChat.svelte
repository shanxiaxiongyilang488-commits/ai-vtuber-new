<script lang="ts">
  ...
  const VIDEO_MODEL_OPTIONS = [
    { id: 'minimax-h3-reference', label: 'MiniMax H3（RunPod RTX 5090）', videoMode: 'production', enabled: true },
  ...
  let modelId = $state('minimax-h3-reference');
  ...
  async function generateAnimation(): Promise<void> {
    ...
    try {
      ...
      if (!confirm(videoCostConfirmation({ label: selectedModel.label, falModel: selectedModel.id }, 5))) {
        status = '動画生成をキャンセルしました。';
        return;
      }
