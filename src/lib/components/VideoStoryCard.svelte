<script lang="ts">
  ...
  $effect(() => {
    if (!videoModelId) videoModelId = initialVideoModelId || 'minimax-h3-reference';
  });
  const selectedVideoModel = $derived(VIDEO_MODELS.find((model) => model.id === videoModelId));
  const selectedVideoProviderLabel = $derived(selectedVideoModel?.provider === 'runpod-h3' ? 'RunPod H3' : selectedVideoModel?.provider === 'minimax' ? 'MiniMax API' : 'FAL');
  ...
  async function sendVideoPrompt(): Promise<void> {
    ...
    videoProgress = 0;
    videoPromptSendStatus = `${selectedVideoModel.label} (${selectedVideoProviderLabel}) の動画生成ジョブを開始しています…`;
    try {
      ...
      const isMiniMaxH3 = selectedVideoModel.provider === 'minimax' || selectedVideoModel.provider === 'runpod-h3';
      ...
      const safeDuration = isMiniMaxH3
        ? Math.min(15, Math.max(4, Math.round(originalDuration)))
        : isSeedanceMini
        ? Math.min(15, Math.max(1, Math.round(originalDuration)))
        : originalDuration;
      if (!confirm(videoCostConfirmation(selectedVideoModel, safeDuration))) {
        videoPromptSendStatus = '動画生成をキャンセルしました。';
      ...
      console.log('[VIDEO_IMAGE_INPUT_CREATED]', { provider: selectedVideoModel.provider, modelId: selectedVideoModel.id, model: selectedVideoModel.falModel, mode: selectedVideoModel.mode, imageUrl, imageCount: referenceImageUrls.length || Number(Boolean(imageUrl)), referenceSource });
      ...
      videoProgress = 100;
      videoPromptSendStatus = `${selectedVideoModel.label} (${selectedVideoProviderLabel}) の動画生成が完了しました。`;
    } catch (error) {
