<script lang="ts">
  import type { MemoryReviewResponse } from '$lib/memoryReview';
  ...
  function recommendCount(): number {
    return review.candidates.filter((candidate, index) => candidate.importance >= 0.8 && !savedIndexes.includes(index) && !skippedIndexes.includes(index)).length;
  }

  function saveLabel(value: number): string {
    if (value >= 0.8) return '保存推奨';
    if (value >= 0.5) return '確認推奨';
    return '保存非推奨';
  }
</script>

<section class="memory-inbox" class:closed={!open} aria-label="Memory Inbox">
  ...
  {#if open}
    <div class="inbox-body">
      {#if review.reflectionComment}
        <p class="character-comment">「{review.reflectionComment}」</p>
      {/if}
      ...
      <div class="review-panel">
        <div class="review-heading">
          ...
          <div>
            <p class="review-label">Memory Review</p>
        ...
        {#if review.candidates.length > 0}
          <div class="candidate-list">
            {#each review.candidates as candidate, index (candidate.title + index)}
              <article class="candidate">
                <div class="candidate-stars" aria-label={`${Math.round(candidate.importance * 100)}%`}>
                  {stars(candidate.importance)}
                </div>
                <div class="candidate-copy">
                  <p class="candidate-title">{candidate.title} <small>{saveLabel(candidate.importance)}</small></p>
        ...
        {:else}
          <p class="empty">今日は長く覚えておく候補はありませんでした。</p>
        {/if}
      </div>
      ...
      {#if developerMode}
        <details class="developer-log">
          ...
          <div class="developer-metrics">
            ...
            <span>Importance: {Math.round(review.importance * 100)}%</span>
            <span>Cost: {review.cost?.estimated ? 'estimated' : '-'}</span>
          ...
          {/if}
          <p class="developer-subhead">Review JSON</p>
...
<style>
  .memory-inbox {
    ...
    background:
      ...
      rgba(2, 6, 23, 0.76);
    box-shadow: 0 0 18px rgba(34, 211, 238, 0.14), inset 0 0 14px rgba(34, 211, 238, 0.04);
    overflow: hidden;
  }
  .memory-inbox.closed {
